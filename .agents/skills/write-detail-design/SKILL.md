...
This is the **Design-stage skill** of the ai-platform workflow (Understand → Design → Develop). It produces a 4-section design doc (name / change points / As-Is & To-Be / tests) grounded in `architecture/` and real code under `.microservices/`, saved to `feature/detail-design/`. The skill reads actual source code to verify every claimed change point and traces one-level call chains to ensure the As-Is description reflects reality.
...
- Asking how something works (that's the Understand stage, not Design)

## How It Works

1. Collect ticket number (hard prerequisite), requirement description, and change-location hints from the user
2. Read all files under `architecture/` (mandatory per AGENT.md)
3. Verify the involved services are already cloned under `.microservices/` — if not, instruct the user to run the `clone-services-from-init` skill
